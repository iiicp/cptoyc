...
raw_ostream::~raw_ostream() {
  ...
  if (Error)
    assert(false && "IO failure on output stream.");
}
...
raw_ostream &raw_fd_ostream::changeColor(enum Colors colors, bool bold,
                                         bool bg) {
  if (CPToyC::Compiler::Process::ColorNeedsFlush())
    flush();
  const char *colorcode =
    (colors == SAVEDCOLOR) ? CPToyC::Compiler::Process::OutputBold(bg)
    : CPToyC::Compiler::Process::OutputColor(colors, bold, bg);
  if (colorcode) {
...
raw_ostream &raw_fd_ostream::resetColor() {
  if (CPToyC::Compiler::Process::ColorNeedsFlush())
    flush();
  const char *colorcode = CPToyC::Compiler::Process::ResetColor();
  if (colorcode) {
    size_t len = strlen(colorcode);
    write(colorcode, len);
    // don't account colors towards output characters
