...
MacroArgs *Preprocessor::ReadFunctionLikeMacroArgs(Token &MacroName,
    ...
    while (Tok.isNot(tok::r_paren)) {
        ...
        while (1) {
            ...
            if (Tok.is(tok::eof) || Tok.is(tok::eom)) { // "#if f(<eof>" & "#if f(\n"
                std::cout << "Diag(MacroName, diag::err_unterm_macro_invoc);" << std::endl;
        ...
        if (!isVariadic && NumFixedArgsLeft == 0) {
            ...
            std::cerr << "Diag(ArgStartLoc, diag::err_too_many_args_in_macro_invoc);" << std::endl;
            return 0;
        }
#if 0
    ...
    if (NumActuals < MinArgsExpected) {
        ...
        if (NumActuals == 0 && MinArgsExpected == 1) {
            ...
            // #define A(x, ...)
            // A("blah")
            std::cerr << "Diag(Tok, diag::ext_missing_varargs_arg);" << std::endl;
            ...
            std::cerr << "Diag(Tok, diag::err_too_few_args_in_macro_invoc);" << std::endl;
            return 0;
        }
        ...
        // Emit the diagnostic at the macro name in case there is a missing ).
        // Emitting it at the , could be far away from the macro name.
        std::cerr << "Diag(MacroName, diag::err_too_many_args_in_macro_invoc);" << std::endl;
        return 0;
    }
