...
void Preprocessor::ReadMacroName(Token &MacroNameTok, char isDefineUndef) {
    ...
    if (MacroNameTok.is(tok::eom)) {
        std::cerr << "Diag(MacroNameTok, diag::err_pp_missing_macro_name);" << std::endl;
        return;
    }
...
#endif
        std::cerr << "Diag(MacroNameTok, diag::err_pp_macro_not_identifier);" << std::endl;
        // Fall through on error.
    } else if (isDefineUndef && II->getPPKeywordID() == tok::pp_defined) {
        // Error if defining "defined": C99 6.10.8.4.
        std::cerr << "Diag(MacroNameTok, diag::err_defined_macro_name);" << std::endl;
    } else if (isDefineUndef && II->hasMacroDefinition() &&
               getMacroInfo(II)->isBuiltinMacro()) {
        // Error if defining "__LINE__" and other builtins: C99 6.10.8.4.
        if (isDefineUndef == 1)
            std::cerr << "Diag(MacroNameTok, diag::pp_redef_builtin_macro);" << std::endl;
        else
            std::cerr << "Diag(MacroNameTok, diag::pp_undef_builtin_macro);" << std::endl;
    } else {
...
//            CurPTHLexer->Lex(Tok);
        ...
        if (Tok.is(tok::eof)) {
            ...
            while (!CurPPLexer->ConditionalStack.empty()) {
                std::cerr << "Diag(CurPPLexer->ConditionalStack.back().IfLoc,diag::err_pp_unterminated_conditional);" << std::endl;
        ...
        if (FirstChar == 'i' && Directive[1] == 'f') {
            ...
            if (IdLen == 5 && !strcmp(Directive+1, "ndif")) {  // "endif"
                ...
                if (CondInfo.FoundElse)
                    std::cerr << "Diag(Tok, diag::pp_err_else_after_else);" << std::endl;
                ...
                if (CondInfo.FoundElse)
                    std::cerr << "Diag(Tok, diag::pp_err_elif_after_else);" << std::endl;
...
void Preprocessor::HandleDirective(Token &Result) {
    ...
    if (InMacroArgs)
        std::cerr << "Diag(Result, diag::ext_embedded_directive);" << std::endl;
    ...
    switch (Result.getKind()) {
        ...
        default:
            ...
            switch (II->getPPKeywordID()) {
                ...
                case tok::pp_warning:
                    std::cerr << "Diag(Result, diag::ext_pp_warning_directive);" << std::endl;
...
static bool GetLineValue(Token &DigitTok, unsigned &Val,
    ...
    if (DigitTok.isNot(tok::numeric_constant)) {
        std::cerr << "PP.Diag(DigitTok, DiagID);" << std::endl;
...
//        PP.Diag(DigitTok, DiagID);
    ...
    if (DigitTokBegin[0] == '0')
        std::cerr << "PP.Diag(DigitTok.getLocation(), diag::warn_pp_line_decimal);" << std::endl;
...
void Preprocessor::HandleLineDirective(Token &Tok) {
    ...
    unsigned LineNo;
    if (GetLineValue(DigitTok, LineNo,*this))
        return;
    ...
    if (LineNo >= LineLimit)
        std::cerr << "Diag(DigitTok, diag::ext_pp_line_too_big) << LineLimit;" << std::endl;
    ...
    if (StrTok.is(tok::eom))
        ...
        std::cerr << "Diag(StrTok, diag::err_pp_line_invalid_filename);" << std::endl;
        DiscardUntilEndOfDirective();
        return;
    } else {
...
static bool ReadLineMarkerFlags(bool &IsFileEntry, bool &IsFileExit,
    ...
    if (GetLineValue(FlagTok, FlagVal, PP))
        return true;
...
void Preprocessor::HandleDigitDirective(Token &DigitTok) {
    ...
    unsigned LineNo;
    if (GetLineValue(DigitTok, LineNo,
                     *this))
        return;
...
//        return CurPTHLexer->DiscardToEndOfLine();
    ...
    if (isWarning) {
        std::cerr << "diag::pp_hash_warning" << std::endl;
        std::cerr << Message << std::endl;
    }
    else {
        std::cerr << "diag::err_pp_hash_error" << std::endl;
        std::cerr << Message << std::endl;
    }
}
...
bool Preprocessor::GetIncludeFilenameSpelling(SourceLocation Loc,
    ...
    if (BufStart[0] == '<') {
        if (BufEnd[-1] != '>') {
            std::cerr << "Diag(Loc, diag::err_pp_expects_filename);" << std::endl;
            BufStart = 0;
            return true;
        }
        isAngled = true;
    ...
    if (BufEnd-BufStart <= 2) {
        std::cerr <<  "Diag(Loc, diag::err_pp_empty_filename);" << std::endl;
        BufStart = 0;
        return "";
    }
...
static bool ConcatenateIncludeName(std::vector<char> &FilenameBuffer,
    ...
    std::cerr << "PP.Diag(CurTok.getLocation(), diag::err_pp_expects_filename);" << std::endl;
    return true;
}
...
void Preprocessor::HandleIncludeDirective(Token &IncludeTok,
    ...
    switch (FilenameTok.getKind()) {
        ...
        case tok::less:
            ...
            break;
        default:
            std::cerr << "Diag(FilenameTok.getLocation(), diag::err_pp_expects_filename);" << std::endl;
            DiscardUntilEndOfDirective();
            return;
    }
    ...
    if (IncludeMacroStack.size() == MaxAllowedIncludeStackDepth-1) {
        std::cerr << "Diag(FilenameTok, diag::err_pp_include_too_deep);" << std::endl;
        return;
    }
    ...
    if (File == 0) {
        std::cerr << "Diag(FilenameTok, diag::err_pp_file_not_found)" << std::endl;
        std::cerr << std::string(FilenameStart, FilenameEnd) << std::endl;
        return;
    }
...
void Preprocessor::HandleIncludeNextDirective(Token &IncludeNextTok) {
    std::cerr << "Diag(IncludeNextTok, diag::ext_pp_include_next_directive);" << std::endl;
    ...
    if (isInPrimaryFile()) {
        Lookup = 0;
        std::cerr << "Diag(IncludeNextTok, diag::pp_include_next_in_primary);" << std::endl;
    } else if (Lookup == 0) {
        std::cerr << "Diag(IncludeNextTok, diag::pp_include_next_absolute_path);" << std::endl;
    } else {
...
void Preprocessor::HandleIncludeMacrosDirective(Token &IncludeMacrosTok) {
    ...
    if (strcmp(SourceMgr.getBufferName(Loc), "<built-in>") != 0) {
        std::cerr << "Diag(IncludeMacrosTok.getLocation()," << std::endl;
        std::cerr << "diag::pp_include_macros_out_of_predefines);" << std::endl;
        DiscardUntilEndOfDirective();
        return;
    }
    ...
    while (1) {
        ...
        switch (Tok.getKind()) {
            ...
            default:
                ...
                if (II == 0) {
                    // #define X(1
                    std::cerr << "Diag(Tok, diag::err_pp_invalid_tok_in_arg_list);" << std::endl;
                    return true;
                }
                ...
                if (std::find(Arguments.begin(), Arguments.end(), II) !=
                    ...
                    std::cerr << "Diag(Tok, diag::err_pp_duplicate_name_in_arg_list) << II;" << std::endl;
                    return true;
                }
                ...
                switch (Tok.getKind()) {
                    ...
                    case tok::ellipsis:  // #define X(A... -> GCC extension
                        // Diagnose extension.
                        std::cerr << "Diag(Tok, diag::ext_named_variadic_macro);" << std::endl;
...
//            Diag(MI->getReplacementToken(NumTokens-1), diag::err_paste_at_end);
    ...
    if (MacroInfo *OtherMI = getMacroInfo(MacroNameTok.getIdentifierInfo())) {
        ...
        if (!SourceMgr.isInSystemHeader(DefineTok.getLocation())) {
            if (!OtherMI->isUsed())
                std::cerr << "Diag(OtherMI->getDefinitionLoc(), diag::pp_macro_not_used);" << std::endl;

            // Macros must be identical.  This means all tokes and whitespace
            // separation must be the same.  C99 6.10.3.2.
            if (!MI->isIdenticalTo(*OtherMI, *this)) {
//                Diag(MI->getDefinitionLoc(), diag::ext_pp_macro_redef)
                  std::cerr  << MacroNameTok.getIdentifierInfo() << std::endl;
...
void Preprocessor::HandleUndefDirective(Token &UndefTok) {
    ...
    if (!MI->isUsed())
        std::cerr << "Diag(MI->getDefinitionLoc(), diag::pp_macro_not_used);" << std::endl;
...
void Preprocessor::HandleIfDirective(Token &IfToken,
    ...
    IdentifierInfo *IfNDefMacro = 0;
    // todo
    bool ConditionalTrue =  true;//EvaluateDirectiveExpression(IfNDefMacro);
...
//        Diag(Result, diag::pp_err_else_without_if);
    ...
    // If this is a #else with a #else before it, report the error.
    if (CI.FoundElse)
        std::cerr << "Diag(Result, diag::pp_err_else_after_else);" << std::endl;
...
void Preprocessor::HandleElifDirective(Token &ElifToken) {
    ...
    PPConditionalInfo CI;
    if (CurPPLexer->popConditionalLevel(CI)) {
        std::cerr << "Diag(ElifToken, diag::pp_err_elif_without_if);" << std::endl;
        return;
    }
    ...
    // If this is a #elif with a #else before it, report the error.
    if (CI.FoundElse)
       std::cerr << "Diag(ElifToken, diag::pp_err_elif_after_else);" << std::endl;
