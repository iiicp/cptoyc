...
namespace CPToyC {
    namespace Compiler {
        ...
        class HeaderSearch {
            ...
            std::unordered_map<std::string, std::pair<unsigned, unsigned >> LookupFileCache;


            /// FrameworkMap - This is a collection mapping a framework or subframework
            /// name like "Carbon" to the Carbon.framework directory.
            //llvm::StringMap<const DirectoryEntry *> FrameworkMap;
            std::unordered_map<std::string, const DirectoryEntry *> FrameworkMap;
...
//                return FrameworkMap.GetOrCreateValue(FWNameStart, FWNameEnd).getValue();
                std::string name(FWNameStart, FWNameEnd);
                if (FrameworkMap.find(name) != FrameworkMap.end()) {
                    return FrameworkMap[name];
                }
                DirectoryEntry *entry = new DirectoryEntry();
                FrameworkMap[name] = entry;
                return reinterpret_cast<const DirectoryEntry *&>(*entry);
            }
            ...
            CharacteristicKind getFileDirFlavor(const FileEntry *File) {
                return (CharacteristicKind)getFileInfo(File).DirInfo;
            }
            ...
            void MarkFileSystemHeader(const FileEntry *File) {
                getFileInfo(File).DirInfo = C_System;
            }
