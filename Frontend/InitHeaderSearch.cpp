...
void InitHeaderSearch::AddPath(const std::string &Path, IncludeDirGroup Group,
  ...
  CharacteristicKind Type;
  if (Group == Quoted || Group == Angled)
    Type = C_User;
  else if (isCXXAware)
    Type = C_System;
  else
    Type = C_ExternCSystem;
...
void InitHeaderSearch::AddDefaultSystemIncludePaths(const LangOptions &Lang) {
  ...
  std::string cinc = "/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk/usr/include";
  AddPath(cinc, System, false, false, false);
...
static void RemoveDuplicates(std::vector<DirectoryLookup> &SearchList,
  ...
  for (unsigned i = 0; i != SearchList.size(); ++i) {
    ...
    if (CurEntry.getDirCharacteristic() != C_User) {
      ...
      if (SearchList[FirstDir].getDirCharacteristic() == C_User)
