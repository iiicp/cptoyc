...
//                ResultToks.append(ResultArgToks, ResultArgToks+NumToks);
        ...
        if (NumToks) {  // Not an empty argument?
            ...
            if (PasteBefore && ResultToks.size() >= 2 &&
                ...
                std::cout << "PP.Diag(ResultToks.back().getLocation(), diag::ext_paste_comma);" << std::endl;
                ResultToks.pop_back();
            }
...
bool TokenLexer::PasteTokens(Token &Tok) {
    ...
    do {
        ...
        if (Tok.is(tok::identifier) && RHS.is(tok::identifier)) {
            ...
            Lexer TL(SourceMgr.getLocForStartOfFile(LocFileID),ScratchBufStart,
