...
//            InitCharacterInfo();
        ...
        Lexer::Lexer(FileID FID, Preprocessor &PP)
            ...
            const MemoryBuffer *InputFile = PP.getSourceManager().getBuffer(FID);
        ...
        Lexer::Lexer(SourceLocation fileloc, const char *BufStart,
        ...
        /// suitable for calls to 'LexRawToken'.  This lexer assumes that the text
        /// range will outlive it, so it doesn't take ownership of it.
        Lexer::Lexer(FileID FID, const SourceManager &SM)
                : FileLoc(SM.getLocForStartOfFile(FID)) {
            const MemoryBuffer *FromFile = SM.getBuffer(FID);
        ...
        unsigned Lexer::MeasureTokenLength(SourceLocation Loc,
            ...
            Lexer TheLexer(Loc, Buffer.first, StrData, Buffer.second);
            Token TheTok;
            ...
            if (Ptr[0] == '\\') {
                ...
                if (unsigned EscapedNewLineSize = getEscapedNewLineSize(Ptr)) {
                    ...
                    if (Ptr[0] != '\n' && Ptr[0] != '\r' && Tok && !isLexingRawMode()) {
                        std::cout << "Diag(Ptr, diag::backslash_newline_space);" << std::endl;
                    }
...
//            }
        ...
        void Lexer::LexIdentifier(Token &Result, const char *CurPtr) {
           ...
           FinishIdentifier:
            ...
            while (1) {
                if (!isIdentifierBody(C)) { // FIXME: UCNs.
        ...
        void Lexer::LexStringLiteral(Token &Result, const char *CurPtr, bool Wide) {
            ...
            while (C != '"') {
                ...
                if (C == '\\') {
                    ...
                    if (!isLexingRawMode())
                        std::cout << "Diag(BufferPtr, diag::err_unterminated_string);" << std::endl;
            ...
            if (NulCharacter && !isLexingRawMode())
                std::cerr << "Diag(NulCharacter, diag::null_in_string);" << std::endl;
        ...
        void Lexer::LexCharConstant(Token &Result, const char *CurPtr) {
            ...
            if (C == '\'') {
                if (!isLexingRawMode())
                    std::cerr << "Diag(BufferPtr, diag::err_empty_character);" << std::endl;
                ...
                do {
                    ...
                    if (C == '\\') {
                        ...
                        C = getAndAdvanceChar(CurPtr, Result);
                               ...
                               (C == 0 && CurPtr-1 == BufferEnd)) {    // End of file.
                        if (!isLexingRawMode())
                            std::cerr << "Diag(BufferPtr, diag::err_unterminated_char);" << std::endl;
                        FormTokenWithChars(Result, CurPtr-1, tok::unknown);
                        return;
                    } else if (C == 0) {
                        NulCharacter = CurPtr-1;
            ...
            if (NulCharacter && !isLexingRawMode())
                std::cerr << "Diag(NulCharacter, diag::null_in_char);" << std::endl;
                ...
                if (CurPtr != OldPtr+1 && C != '/' && CurPtr[0] != '/') {
                    for (; OldPtr != CurPtr; ++OldPtr)
                        if (OldPtr[0] == '\n' || OldPtr[0] == '\r') {
                            ...
                            if (!isLexingRawMode()) {
                                std::cout << "Diag(OldPtr - 1, diag::ext_multi_line_bcpl_comment);"<<std::endl;
                            }
                            break;
                        }
                }
...
//                }
                if (!L->isLexingRawMode()) {
                    std::cout << "L->Diag(CurPtr, diag::trigraph_ends_block_comment);" << std::endl;
                }
            }

            // Warn about having an escaped newline between the */ characters.
            if (!L->isLexingRawMode()) {
                std::cout << "L->Diag(CurPtr, diag::escaped_newline_block_comment_end);" << std::endl;
            }

            // If there was space between the backslash and newline, warn about it.
            if (HasSpace && !L->isLexingRawMode()) {
                std::cout << "L->Diag(CurPtr, diag::backslash_newline_space);" << std::endl;
            }
            return true;
        }
        ...
        bool Lexer::SkipBlockComment(Token &Result, const char *CurPtr) {
            ...
            if (C == 0 && CurPtr == BufferEnd+1) {
                if (!isLexingRawMode()) {
                    std::cout << "Diag(BufferPtr, diag::err_unterminated_block_comment);"<<std::endl;
                }
                --CurPtr;
            ...
            while (1) {
                ...
                if (C == '/') {
                    ...
                    if (CurPtr[0] == '*' && CurPtr[1] != '/') {
                        ...
                        if (!isLexingRawMode()) {
                            std::cout << "Diag(CurPtr - 1, diag::warn_nested_block_comment);"<<std::endl;
                        }
                    }
        ...
        bool Lexer::LexEndOfFile(Token &Result, const char *CurPtr) {
            ...
            while (!ConditionalStack.empty()) {
                std::cerr << "PP->Diag(ConditionalStack.back().IfLoc,diag::err_pp_unterminated_conditional);" << std::endl;
            ...
            if (CurPtr != BufferStart && (CurPtr[-1] != '\n' && CurPtr[-1] != '\r')) {
                std::cerr << "Diag(BufferEnd, diag::ext_no_newline_eof) " << std::endl;
                std::cerr << "CodeModificationHint::CreateInsertion(getSourceLocation(BufferEnd),);" << std::endl;
            }
        ...
        void Lexer::LexTokenInternal(Token &Result) {
            ...
            switch (Char) {
                case 0: {
                    ...
                    if (!isLexingRawMode()) {
                        std::cout << "Diag(CurPtr - 1, diag::null_in_file);" << std::endl;
                    }
                    Result.setFlag(Token::LeadingSpace);
