...
const MemoryBuffer *ContentCache::getBuffer() const {
...
void SourceManager::AddLineNote(SourceLocation Loc, unsigned LineNo,
    ...
    const FileInfo &FileInfo = getSLocEntry(LocInfo.first).getFile();

    // Remember that this file has #line directives now if it doesn't already.
    const_cast<class FileInfo&>(FileInfo).setHasLineDirectives();
...
SourceManager::getOrCreateContentCache(const FileEntry *FileEnt) {
    ...
    if (FileInfos.find(FileEnt) != FileInfos.end()) {
        return FileInfos[FileEnt];
    }
    ...
    FileInfos.insert(std::make_pair(FileEnt, new ContentCache(FileEnt)));
    return FileInfos[FileEnt];
}
...
SourceManager::getBufferData(FileID FID) const {
    const MemoryBuffer *Buf = getBuffer(FID);
...
FileID SourceManager::getFileIDSlow(unsigned SLocOffset) const {
    ...
    std::vector<SLocEntry>::const_iterator I;
...
SourceManager::getDecomposedInstantiationLocSlowCase(const SLocEntry *E,
...
std::pair<FileID, unsigned>
SourceManager::getDecomposedSpellingLocSlowCase(const SLocEntry *E,
                                                unsigned Offset) const {
    // If this is an instantiation record, walk through all the instantiation
    // points.
    FileID FID;
...
static void ComputeLineNumbers(ContentCache* FI){
    ...
    FI->SourceLineCache = (unsigned *)malloc(LineOffsets.size() * sizeof(unsigned));
...
SourceManager::getFileCharacteristic(SourceLocation Loc) const {
    ...
    std::pair<FileID, unsigned> LocInfo = getDecomposedInstantiationLoc(Loc);
    const FileInfo &FI = getSLocEntry(LocInfo.first).getFile();
...
PresumedLoc SourceManager::getPresumedLoc(SourceLocation Loc) const {
    ...
    const FileInfo &FI = getSLocEntry(LocInfo.first).getFile();
    const ContentCache *C = FI.getContentCache();
...
bool SourceManager::isBeforeInTranslationUnit(SourceLocation LHS,
    ...
    while (1) {
        ...
        const SLocEntry &Entry = getSLocEntry(ROffs.first);
    ...
    while (1) {
        SourceLocation UpperLoc;
        const SLocEntry &Entry = getSLocEntry(LOffs.first);
        if (Entry.isInstantiation())
            UpperLoc = Entry.getInstantiation().getInstantiationLocStart();
        else
            UpperLoc = Entry.getFile().getIncludeLoc();
...
void SourceManager::PrintStats() const {
    ...
    std::cerr << FileInfos.size() << " files mapped, " << MemBufferInfos.size()
              << " mem buffers mapped.\n";
    std::cerr << SLocEntryTable.size() << " SLocEntry's allocated, "
    ...
    std::cerr << "FileID scans: " << NumLinearScans << " linear, "
